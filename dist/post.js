...
function expandGlobPath(pathStr) {
  try {
    const globPattern = path2__default.default.resolve(CWD, pathStr);
...
var getVars = () => {
  ...
  for (const pathStr of options.paths) {
    ...
    if (isGlobPattern(pathStr)) {
      ...
      const relativePath = path2__default.default.relative(CWD, targetPath);
      const cachePath = path2__default.default.join(cacheDir, relativePath);
      ...
      const { dir: targetDir } = path2__default.default.parse(targetPath);
      pathItems.push({
        cachePath,
        targetDir,
        targetPath
...
async function processPathItem(pathItem, cacheDir, strategy) {
  ...
  const cacheParentDir = path2__default.default.dirname(cachePath);
